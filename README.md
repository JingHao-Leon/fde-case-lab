...
**真实 FDE 案例的可运行工程实现集 + AI Agent 技能包**
...
- **合成数据为主**：17 个项目中仅 2 个（案例 08 物料匹配、22 补货回测）使用真实公开数据，
  其余案例（含案例 23 的审单样例）均为固定种子合成数据——核心业务数据为企业私有、
  从未公开（详见[数据说明](#数据说明诚实边界)）。指标验证的是方法与工程模式，
  不能直接外推为真实企业数据上的表现；"老师傅基线"等对照项按案例陈述的行为建模。
- **离线回测，非生产验证**：所有指标均为离线回测，不代表生产环境表现；未覆盖并发、
  延迟、权限、数据接入与回写等生产工程问题。规模为演示量级（如 200 SKU × 240 天
  零售日需求、2,293 对实体匹配标注、案例 23 仅 2 行明细的审单样例）。
- **检索类案例不含生成环节**：案例 01 / 08 的检索评测基于 TF-IDF / BM25 等传统方法
  （`cases/01-knowledge-base/solution.py`），不涉及 LLM 生成，也没有端到端答案质量
  评测——"RAG"覆盖的是检索（R）与检索评测部分。
- **LLM 案例存在外部依赖**：案例 23 的单据抽取环节依赖 LLM API（成本、延迟与供应商
  变动由使用者承担），飞书落地依赖 pi agent 与 `pi-feishu-lark` WebSocket 长连接；
  仓库内回测覆盖的是 23 条确定性规则部分（数字判断不经过 LLM）。
- **启发式不保证最优**：3D 装箱（案例 06）为启发式搜索而非精确求解，实测首车装载
  97.1 方（最低 95.8）仍低于原案例宣称的稳定 99 方，根因与改进方向记录在案例 README。
- **技能包未经多运行时验证**：10 个 SKILL.md 为方法论提炼文档，仓库内没有针对技能的
  自动化测试；仅声明与 FDEOps 的 skills 安装约定兼容，未在全部 agent 运行时逐一验证。
